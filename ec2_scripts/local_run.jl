...
function getsolver(solvername, tlim, logl, rgap)
    solvermap = Dict(
        ...
        # "PAJ_MSD_GUROBI_MOSEK_PA" =>
        # PajaritoSolver(mip_solver=GurobiSolver(OutputFlag=1, Threads=1, TimeLimit=tlim, MIPGap=0., IntFeasTol=1e-9, FeasibilityTol=1e-9), cont_solver=MosekSolver(LOG=1, NUM_THREADS=1, OPTIMIZER_MAX_TIME=120., INTPNT_CO_TOL_REL_GAP=1e-11), log_level=3, timeout=tlim, rel_gap=rgap, mip_solver_drives=true, prim_cuts_assist=true),
        # "PAJ_GUROBI_ECOS" =>
...
println("starting compile solve")
...
println("finished compile solve")
